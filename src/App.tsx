...
const terminalThemes = {
  maestr: { background: "#171815", foreground: "#d8dbd1", cursor: "#e7b75e", selectionBackground: "#4a4e43" },
  onedark: { background: "#282c34", foreground: "#abb2bf", cursor: "#61afef", selectionBackground: "#3e4451" },
  catppuccin: { background: "#181825", foreground: "#cdd6f4", cursor: "#cba6f7", selectionBackground: "#45475a" },
  nord: { background: "#2e3440", foreground: "#d8dee9", cursor: "#88c0d0", selectionBackground: "#434c5e" },
  solarized: { background: "#002b36", foreground: "#839496", cursor: "#b58900", selectionBackground: "#073642" },
} satisfies Record<string, TerminalAppearance["theme"]>;
const defaultTerminalAppearance: TerminalAppearance = { fontFamily: '"SFMono-Regular", "Cascadia Code", monospace', fontSize: 12, theme: terminalThemes.maestr };
...
function AgentPanel({ appearance, compact, onToggleCompact, onSessionChange, onAgentChange, onLog }: { appearance: TerminalAppearance; compact: boolean; onToggleCompact: () => void; onSessionChange: (sessionId: number | null) => void; onAgentChange: (agent: string | null) => void; onLog: (message: string) => void }) {
...
function App() {
  ...
  const agentTerminalAppearance = useMemo<TerminalAppearance>(() => ({ fontFamily: String(settings.agentTerminalFont ?? defaultTerminalAppearance.fontFamily), fontSize: Math.max(10, Math.min(24, Number(settings.agentTerminalFontSize ?? 12))), theme: terminalThemes[String(settings.agentTerminalTheme) as keyof typeof terminalThemes] ?? terminalThemes.maestr }), [settings.agentTerminalFont, settings.agentTerminalFontSize, settings.agentTerminalTheme]);
  ...
  const sendComments = async (sessionId: number) => {
    ...
    await invoke("write_terminal", { sessionId, data: `Implement these code review comments now. Paths are relative to the workspace. Inspect the current file around each line, apply the requested changes without asking for confirmation, then summarize each item. Reply to each comment in the same language as that comment. Comments: ${JSON.stringify(payload)}\r` });
  ...
  const generateCommitMessage = async () => {
    ...
    return invoke<string>("generate_commit_message", { agent: selectedAgent, model: String(settings.commitModel ?? "").trim() || null });
  };
  ...
  return (
    <main className={`app-shell theme-${settings.theme ?? "maestr"}`}>
      ...
      {settingsOpen && <div className="settings-backdrop" onClick={() => setSettingsOpen(false)}><section className="settings-panel" onClick={(event) => event.stopPropagation()}><div className="settings-heading"><strong>Settings</strong><button className="icon-button small" onClick={() => setSettingsOpen(false)}><X size={14} /></button></div><div className="settings-section first">Editor</div><label>Font<select value={String(settings.font ?? "SFMono-Regular")} onChange={(event) => updateSettings("font", event.target.value)}><option>SFMono-Regular</option><option>Cascadia Code</option><option>JetBrains Mono</option><option>Fira Code</option></select></label><label>Font size<input type="number" min="10" max="24" value={Number(settings.fontSize ?? 13)} onChange={(event) => updateSettings("fontSize", Number(event.target.value))} /></label><label>Line height<input type="number" min="16" max="40" value={Number(settings.lineHeight ?? 22)} onChange={(event) => updateSettings("lineHeight", Number(event.target.value))} /></label><label>Vertical padding<input type="number" min="0" max="48" value={Number(settings.editorPadding ?? 18)} onChange={(event) => updateSettings("editorPadding", Number(event.target.value))} /></label><label>Whitespace<select value={String(settings.renderWhitespace ?? "selection")} onChange={(event) => updateSettings("renderWhitespace", event.target.value)}><option value="none">Hidden</option><option value="selection">Selection</option><option value="boundary">Boundary</option><option value="all">All</option></select></label><label className="settings-check"><input type="checkbox" checked={settings.lineNumbers !== false} onChange={(event) => updateSettings("lineNumbers", event.target.checked)} /> Show line numbers</label><label className="settings-check"><input type="checkbox" checked={settings.wordWrap === true} onChange={(event) => updateSettings("wordWrap", event.target.checked)} /> Wrap long lines</label><label className="settings-check"><input type="checkbox" checked={settings.minimap === true} onChange={(event) => updateSettings("minimap", event.target.checked)} /> Show minimap</label><label className="settings-check"><input type="checkbox" checked={settings.showHiddenFiles === true} onChange={(event) => updateSettings("showHiddenFiles", event.target.checked)} /> Show hidden files</label><div className="settings-section">Commit generation</div><label>Commit model<input type="text" value={String(settings.commitModel ?? "")} placeholder="Default model" onChange={(event) => updateSettings("commitModel", event.target.value)} /></label><div className="settings-section">Agent terminal</div><label>Font<select value={String(settings.agentTerminalFont ?? defaultTerminalAppearance.fontFamily)} onChange={(event) => updateSettings("agentTerminalFont", event.target.value)}><option value={'"SFMono-Regular", "Cascadia Code", monospace'}>SF Mono</option><option value={'"Cascadia Code", monospace'}>Cascadia Code</option><option value={'"JetBrains Mono", monospace'}>JetBrains Mono</option><option value={'"Fira Code", monospace'}>Fira Code</option></select></label><label>Font size<input type="number" min="10" max="24" value={Number(settings.agentTerminalFontSize ?? 12)} onChange={(event) => updateSettings("agentTerminalFontSize", Number(event.target.value))} /></label><label>Theme<select value={String(settings.agentTerminalTheme ?? "maestr")} onChange={(event) => updateSettings("agentTerminalTheme", event.target.value)}><option value="maestr">Maestr</option><option value="onedark">One Dark</option><option value="catppuccin">Catppuccin Mocha</option><option value="nord">Nord</option><option value="solarized">Solarized Dark</option></select></label><div className="settings-section">Appearance</div><label>Editor theme<select value={String(settings.theme ?? "maestr")} onChange={(event) => updateSettings("theme", event.target.value)}><option value="maestr">Maestr</option><option value="onedark">One Dark</option><option value="catppuccin">Catppuccin Mocha</option><option value="nord">Nord</option><option value="solarized">Solarized Dark</option><option value="light">Light</option><option value="high-contrast">High contrast</option></select></label><label>Diff style<select value={String(settings.diff ?? "side")} onChange={(event) => updateSettings("diff", event.target.value)}><option value="side">Side by side</option><option value="inline">Inline</option></select></label></section></div>}
